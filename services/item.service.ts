...
import { UpdateItemDto, CreateItemDto } from "../types/item.dto.ts";

class ItemService {
  public static async createOne(options: CreateItemDto) {
    const { name, code } = options;
    const orgExists = await Item.findOne({ name });
    if (orgExists) {
    ...
    const user = await Item.insertOne({
      name,
      code,
      isDisabled: false,
      createdAt: createdAt,
  ...
  public static async removeOne(id: string) {
    ...
    const deleteCount: number = await Item.deleteOne({
      _id: new Bson.ObjectId(id),
    });
    if (deleteCount) {
